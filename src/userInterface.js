...
export default class UI {
  ...
  static loadTasks(projectName) {
    Storage.getTodoList()
      ...
      .getTasks()
      .forEach((task) =>
        UI.createTask(task.name, task.dueDate, task.completed)
      );
    ...
    if (projectName !== "Today" && projectName !== "This week") {
      projectPreview.innerHTML += `
        ...
        <div class="add-task-popup" id="add-task-popup">
          ...
          />
          <input
            class="input-add-due-date-popup"
            id="input-add-due-date-popup"
            type="date"
  ...
  static createTask(name, dueDate, completed) {
    ...
    tasksList.innerHTML += `
      <button class="button-task" data-task-button>
        <div class="left-task-panel">
          <i class="far ${completedMark}"></i>
          <p class="task-content ${completedClass}" id="task-content">${name}</p>
  ...
  static initAddTaskButtons() {
    ...
    const addDueDatePopupInput = document.getElementById(
      "input-add-due-date-popup"
    );

    addTaskButton.addEventListener("click", UI.openAddTaskPopup);
    addTaskPopupButton.addEventListener("click", () => {
      UI.addTask(addTaskPopupInput.value, addDueDatePopupInput.value);
    });
  ...
  static addTask() {
    ...
    const taskNameInput = addTaskPopupInput.value;

    if (addDueDatePopupInput.value === "") {
      var dueDate = "No date";
    } else {
      var dueDate = format(new Date(addDueDatePopupInput.value), "dd/MM/yyyy");
    }
    ...
    if (Storage.getTodoList().getProject(projectName).contains(taskNameInput)) {
      alert("Task names must be different");
      addTaskPopupInput.value = "";
      return;
    }

    Storage.addTask(projectName, new Task(taskNameInput, dueDate));
    UI.createTask(taskNameInput, dueDate);
    UI.closeAddTaskPopup();
  }
  ...
  static toggleTaskCompleted(taskButton) {
    const projectName = document.getElementById("project-name").textContent;
    const taskNamePara = taskButton.querySelector(".task-content");
    const taskName = taskNamePara.textContent;
    const taskCompletedIcon = taskButton.querySelector(".far");
    const isCompleted = taskCompletedIcon.classList.contains("fa-circle-check");

    if (isCompleted) {
      taskNamePara.classList.remove("completed");
      taskCompletedIcon.classList.remove("fa-regular", "fa-circle-check");
      taskCompletedIcon.classList.add("fa-circle");
    } else {
      taskNamePara.classList.add("completed");
      taskCompletedIcon.classList.add("fa-regular", "fa-circle-check");
      taskCompletedIcon.classList.remove("fa-circle");
    }

    if (projectName === "Today" || projectName === "This week") {
      const mainProjectName = taskName.split("(")[1].split(")")[0];
      const mainTaskName = taskName.split(" (")[0];
      Storage.toggleTaskCompleted(projectName, taskName);
      Storage.toggleTaskCompleted(mainProjectName, mainTaskName);
  ...
  static setTaskDate() {
    const projectName = document.getElementById("project-name").textContent;
    const taskName =
      this.parentNode.parentNode.children[0].children[1].textContent;
